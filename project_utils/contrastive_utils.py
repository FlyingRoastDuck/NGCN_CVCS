...
def extract_features(model, data_loader, print_freq=50, 
    ...
    with torch.no_grad():
        for i, _item in enumerate(data_loader):
            imgs = _item[0]
            targets = _item[1] # labels
            if_train = _item[3][:, 0].bool() # whether the sample is labeled or unlabeled
...
def info_nce_logits(features, args):
    b_ = features.size(0) // 2  # features:[B*2/expert_num] , b_: B/expert_num
    ...
    # images and their counterparts
    similarity_matrix = torch.matmul(features, features.T)

    # discard the main diagonal from both: labels and similarities matrix
    mask = torch.eye(labels.shape[0], dtype=torch.bool).to(args.device)
    
    labels = labels[~mask].view(labels.shape[0], -1) # remove self, still has same pid
    similarity_matrix = similarity_matrix[~mask].view(similarity_matrix.shape[0], -1) # remove self

    # select counterparts for training
    ...
    return logits / args.temperature, labels
