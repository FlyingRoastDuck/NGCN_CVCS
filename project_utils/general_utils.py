...
def cal_confidence(features, labels, knn=10): 
    affin = features @ features.T
    indicator = labels.view(-1, 1).eq(labels.view(1, -1)).numpy()
    confi = np.zeros(affin.shape[0])
    for idx in range(affin.shape[0]):
        current = affin[idx]
        affin_idx = current.argsort()[::-1]
        affin_idx = affin_idx[:min(knn, affin_idx.shape[0])]
        curr_mask = indicator[idx][affin_idx]
        knn_aff = affin[idx][affin_idx]
        # remove self
        curr_mask, knn_aff = curr_mask[1:], knn_aff[1:]
        confi[idx] = (knn_aff[curr_mask].sum() - knn_aff[~curr_mask].sum()) / (knn-1)
    return torch.from_numpy(confi)
