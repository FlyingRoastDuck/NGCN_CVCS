...
class K_Means:
    ...
    def fit_mix_once(self, u_feats, l_feats, l_targets, random_state):
        ...
        l_classes = torch.unique(l_targets) # labeled
        ...
        centers[:len(l_classes)] = l_centers # assign labeled centers
        ...
        for it in range(self.max_iterations):
            centers_old = centers.clone()
            # dist between unlabeled features and all centroids (lab + unlab)
            dist = pairwise_distance(u_feats, centers, self.pairwise_batch_size)
            u_mindist, u_labels = torch.min(dist, dim=1) # assign labels based on distance
            u_inertia = u_mindist.sum()
            # get labeled sampels and their centroids distances
            l_mindist = torch.sum((l_feats - centers[labels[:l_num]])**2, dim=1)
            l_inertia = l_mindist.sum()
            inertia = u_inertia + l_inertia
            # assign unlabeled sampels labeles
            labels[l_num:] = u_labels

            # update centers based on assigned labels
            for idx in range(self.k):
                selected = torch.nonzero(labels == idx).squeeze()
